...
class S3SnowflakeOperator(BaseOperator):
    ...
    def _execute(
        ...
        **kwargs,
        ...
        # Resources
        s3_resource: S3Resource = kwargs.get("source_resource")
        snow_resource: SnowflakeResource = kwargs.get("target_resource")
        ...
        if file_format_type == "CSV" and source_config.csv_options:
            delimiter = (
                source_config.csv_options.delimiter or source_config.delimiter or ","
            )
            if match_columns:
        ...
        delimiter = (
            (source_config.csv_options.delimiter if source_config.csv_options else None)
            or source_config.delimiter
            or ","
        )
    ...
    def _apply_schema_strategy(
        self,
        context,
        snow_resource: SnowflakeResource,
        ...
        if not table_exists and strategy != "create":
            raise ValueError(
                f"Table {table} does not exist and schema_strategy is '{strategy}' (not 'create')."
            )
        ...
        if file_format_type == "CSV":
            ...
            try:
                df_sample = s3_resource.read_csv_sample(
                    bucket_name, s3_path, delimiter=delimiter
                )
                ...
                context.log.warn(
                    f"Failed to infer CSV schema via Pandas: {e}. Falling back to VARCHAR for all columns."
                )
            ...
            snow_resource.execute_query(
                f"CREATE OR REPLACE FILE FORMAT {temp_format_name} {file_format_sql}"
            )
            try:
                infer_sql = f"""
                ...
                inferred_columns = {
                    row["COLUMN_NAME"].upper(): row["TYPE"].upper()
                    for row in inferred_schema_list
                }
            finally:
                snow_resource.execute_query(
                    f"DROP FILE FORMAT IF EXISTS {temp_format_name}"
                )

        if not inferred_columns:
            context.log.warn(
                f"No columns inferred from @{stage}/{s3_path}. Skipping schema management."
            )
            return
        ...
        if not table_exists:
            # Create Table
            cols_sql = ", ".join(
                [f"{name} {dtype}" for name, dtype in inferred_columns.items()]
            )
            ...
            new_cols = {
                name: dtype
                for name, dtype in inferred_columns.items()
            ...
            if new_cols:
                if strategy == "evolve":
                    for col_name, col_type in new_cols.items():
                        context.log.info(
                            f"Evolving schema: Adding column {col_name} ({col_type}) to {table}"
                        )
                        snow_resource.add_table_column(table, col_name, col_type)
                elif strategy == "strict":
                    raise ValueError(
                        f"Schema mismatch detected in 'strict' mode. New columns found: {list(new_cols.keys())}"
                    )

            if strategy == "strict":
                missing_in_file = set(existing_columns.keys()) - set(
                    inferred_columns.keys()
                )
                if missing_in_file:
                    raise ValueError(
                        f"Schema mismatch detected in 'strict' mode. Columns missing in file: {list(missing_in_file)}"
                    )
