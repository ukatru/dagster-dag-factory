...
class ParquetConfig(BaseConfigModel):
    ...
    schema_overrides: Dict[str, str] = Field(
        default_factory=dict, description="Schema overrides (col_name: pyarrow_type)"
    )
    compression: ParquetCompression = Field(
        default=ParquetCompression.SNAPPY, description="Compression type"
    )
    cast_decimal_to_float: bool = Field(
        default=False, description="Whether to cast decimal to float"
    )
