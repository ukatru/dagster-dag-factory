...
class SFTPConfig(BaseConfigModel):
    ...
    file_name: Optional[str] = Field(default=None, description="Specific file name to match (overrides pattern). Supports templating.")
    ...
    check_is_modifying: bool = Field(default=False, description="Check if file is still being modified before transfer")
    compress_options: Optional[CompressConfig] = Field(default=None, description="Compression settings for upload")
