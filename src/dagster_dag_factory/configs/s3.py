...
class S3Config(BaseConfigModel):
    ...
    object_type: S3ObjectType = Field(
        default=S3ObjectType.CSV, description="S3 Object type (CSV, JSON, PARQUET)"
    )

    mode: S3Mode = Field(
        default=S3Mode.COPY, description="Execution mode (COPY, MULTI_FILE)"
    )
    min_size: int = Field(
        default=5, description="Minimum size threshold in MB (if applicable)"
    )

    @property
    def multi_file(self) -> bool:
        return self.mode == S3Mode.MULTI_FILE

    prefix: str = Field(default="", description="Key prefix for S3 operations")
    pattern: Optional[str] = Field(
        default=None, description="Pattern for file matching (often defaults to key)"
    )
    ...
    csv_options: Optional[CsvConfig] = Field(
        default=None, description="CSV specific options"
    )
    parquet_options: Optional[ParquetConfig] = Field(
        default=None, description="Parquet specific options"
    )
    json_options: Optional[JSONOption] = Field(
        default=None, description="JSON specific options"
    )
