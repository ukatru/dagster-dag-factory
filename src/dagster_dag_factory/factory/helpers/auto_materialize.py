...
def get_auto_materialize_policy(config: Optional[Dict[str, Any]]) -> Optional[AutoMaterializePolicy]:
    """
    ...
    if policy_type == "eager":
        ...
        raise ValueError(f"Unsupported auto_materialize_policy type: {policy_type}. Use 'eager' or 'lazy'.")
    
    # Apply rate limiting if specified
    if max_materializations_per_minute:
        policy = policy.with_max_materializations_per_minute(max_materializations_per_minute)
