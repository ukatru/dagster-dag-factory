...
class ScheduleFactory:
    def create_schedules(
        ...
        for sched_conf in schedules_config:
            ...
            if (
                not is_partitioned
                and hasattr(job, "partitions_def")
                and job.partitions_def
            ):
                is_partitioned = True
            ...
            if is_partitioned and p_def:
                ...
                if is_time_partitioned:
                    ...
                    schedules.append(
                        build_schedule_from_partitioned_job(name=name, job=job)
                    )
                else:
                    # For non-time partitions (static, dynamic), we need the cron
                    schedules.append(
                        build_schedule_from_partitioned_job(
                            name=name, job=job, cron_schedule=cron_schedule
                        )
                    )
            else:
                # Non-partitioned job
                schedules.append(
                    ScheduleDefinition(name=name, job=job, cron_schedule=cron_schedule)
                )
        return schedules
