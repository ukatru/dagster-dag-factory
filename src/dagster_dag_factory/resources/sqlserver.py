...
class SQLServerResource(BaseConfigurableResource):
    ...
    trust_server_certificate: bool = Field(
        default=True, description="Trust server certificate (useful for self-signed)"
    )
    ...
    @contextmanager
    def get_cursor(self) -> Iterator[pyodbc.Cursor]:
        """Yields a raw pyodbc cursor from a new connection."""
    ...
    def execute_query(
        ...
        If a cursor is provided, it uses it; otherwise it creates a temporary one.
        """
        logger = get_dagster_logger()
        ...
        if cursor:
            return self._execute_with_cursor(cursor, sql, params)
        else:
            with self.get_cursor() as new_cursor:
                return self._execute_with_cursor(new_cursor, sql, params)

    def _execute_with_cursor(
        self, cursor: pyodbc.Cursor, sql: str, params: Optional[tuple] = None
    ) -> list:
        """Internal helper to execute and fetch from a cursor."""
        if params:
