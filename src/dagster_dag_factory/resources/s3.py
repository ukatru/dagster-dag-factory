from typing import Optional, Any, List, ClassVar, Union, Callable
import boto3
import os
import uuid
import threading
...
class S3Resource(BaseConfigurableResource):
    ...
    def get_session(self) -> boto3.Session:
        ...
        if self.assume_role_arn:
            ...
            role_session_name = (
                self.assume_role_session_name or f"dagster-{uuid.uuid4()}"
            )
    ...
    def get_client(self) -> Any:
        ...
        return session.client(
            "s3", endpoint_url=self.endpoint_url, verify=self.verify, config=config
        )
