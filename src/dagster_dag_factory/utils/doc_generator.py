...
import dagster_dag_factory.operators # Trigger registration
...
def generate_docs(output_path: str):
    ...
    for name, obj in inspect.getmembers(resources_module):
        if inspect.isclass(obj) and issubclass(obj, ConfigurableResource) and obj != ConfigurableResource:
            lines.append(f"### `{name}`")
            if obj.__doc__:
    ...
    for (source, target), op_class in OperatorRegistry._registry.items():
        ...
        if op_class.source_config_schema:
            ...
            lines.append("| Field | Type | Description |")
            lines.append("| :--- | :--- | :--- |")
            props = op_class.source_config_schema.model_json_schema().get("properties", {})
            for name, d in props.items():
                lines.append(f"| `{name}` | `{d.get('type','Any')}` | {d.get('description','')}|")
            lines.append("\n")
        ...
        if op_class.target_config_schema:
            ...
            lines.append("| Field | Type | Description |")
            lines.append("| :--- | :--- | :--- |")
            props = op_class.target_config_schema.model_json_schema().get("properties", {})
            for name, d in props.items():
                lines.append(f"| `{name}` | `{d.get('type','Any')}` | {d.get('description','')}|")
            lines.append("\n")
